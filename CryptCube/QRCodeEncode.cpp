...
/////////////////////////////////////////////////////////////////////////////
// QR绳子或代码版本(类型轮)信息
static QR_VERSIONINFO QR_VersonInfo[] = {{0}, // (伪代码:Ver.0)
										 { 1, // Ver.1
										    26,   19,   16,   13,    9,
										   0,   0,   0,   0,   0,   0,   0,
										   1,  26,  19,
...
//用途： 数据编码
//参数： 错误校正的水平，类型轮(0=自动)，类型轮自动延长的旗子，掩码的数字(- 1=自动地工作)，输入的数据，输入的数据长的
//返回值： 在编码成功=TRUE之时，随着时间的过去没有数据或者容量=FALSE
BOOL CQR_Encode::EncodeData(int nLevel, int nVersion, BOOL bAutoExtent, int nMaskingNo, LPCSTR lpsSource, int ncSource)
{
	...
	//当数据长度没有被任命时，获取ncLength。由
	//QRCode的汉字模式使用GB2312双字节字符集，因此这里必须使用lstrlenA。
	...
	else
	{
		...
		{
			if (bAutoExtent)
				m_nVersion = nEncodeVersion; //版本(类型轮)自动地工作扩展
			else		
				m_nVersion = nVersion; //return FALSE; //容量 		//  [10/21/2010 Zero.Hu]
		}
	}
...
BOOL CQR_Encode::EncodeSourceData(LPCSTR lpsSource, int ncLength, int nVerGroup)
{
	...
	/////////////////////////////////////////////////////////////////////////
	// 它毗邻与情况连接字母数字方式块和数字方式块的线 

	int ncSrcBits, ncDstBits; // 当它阻拦时，前位长度和选拔字母数字方式位长度

	int nBlock = 0;

	while (nBlock < m_ncDataBlock - 1)
	{
		int ncJoinFront, ncJoinBehind; // 近似位长度，当它连接用8位字节方式块
		int nJoinPosition = 0; // 与8位字节方式块的连接： -它连接用在1=之前， 0=不连接，它连接用1=后方
			...
			(m_byBlockMode[nBlock] == QR_MODE_ALPHABET && m_byBlockMode[nBlock + 1] == QR_MODE_NUMERAL))
		{
			...
			if (ncSrcBits > ncDstBits)
			{
				...
				{
					// 当近似地有8位字节方式块两个，更喜欢那个数据长度变得短的地方
	...
	while (nBlock < m_ncDataBlock - 1)
	{
		...
		// 当有8位字节方式块前面，重叠减法的显示数额
		if (nBlock >= 1 && m_byBlockMode[nBlock - 1] == QR_MODE_8BIT)
			ncDstBits -= (4 + nIndicatorLen8Bit[nVerGroup]);

		// 当有8位字节方式块在后方，重叠减法的显示数额
		if (nBlock < m_ncDataBlock - 2 && m_byBlockMode[nBlock + 2] == QR_MODE_8BIT)
			ncDstBits -= (4 + nIndicatorLen8Bit[nVerGroup]);
	...
	/////////////////////////////////////////////////////////////////////////
	// 位划船
	int ncComplete = 0; // 处理结尾数据逆
	WORD wBinCode;

	m_ncDataCodeWordBit = 0; // 位单位处理逆
...
//用途： 被咬住的长的承购
 //论据： 数据方式分类和数据长度，版本(类型轮)小组
 //返回值： 数据位元长的
 //评论： 关于数据长的论据以不是汉字方式信件的数量，字节数
...
//用途： 位集合
 //论据： 插入位置和位安排数据，数据位元长的(最多16)
 //返回值： 下插入位置(当缓冲结束=-1)
 //评论： 在m_byDataCodeWord结果设置了(要点零初始化)
...
//用途： RS错误校正的绳子或暗语承购
 //论据： 数据捆绑或暗语地址、数据绳子或者暗语长度、RS绳子或者暗语长的
 //评论： 在保证整个绳子或暗语数额区域以后，电话
...
void CQR_Encode::GetRSCodeWord(LPBYTE lpbyRSWork, int ncDataCodeWord, int ncRSCodeWord)
{
	...
	for (i = 0; i < ncDataCodeWord ; ++i)
	{
		if (lpbyRSWork[0] != 0)
		{
			BYTE nExpFirst = byIntToExp[lpbyRSWork[0]]; // 从初始项因素乘算器演算

			for (j = 0; j < ncRSCodeWord; ++j)
			{
				// 在每部分乘算器初始项乘算器加法(% 55 - > α^255 = 1)
...
void CQR_Encode::FormatModule()
{
	...
	if (m_nMaskingNo == -1)
	{
		...
		SetMaskingPattern(m_nMaskingNo); // 掩码
		SetFormatInfoPattern(m_nMaskingNo); // 格式信息图设置

		int nMinPenalty = CountPenalty();

		for (i = 1; i <= 7; ++i)
		{
			SetMaskingPattern(i); // 掩码
			SetFormatInfoPattern(i); // 格式信息图设置
...
void CQR_Encode::SetCodeWordPattern()
{
	...
	int nCoef_x = 1; // x轴向设置方向
	int nCoef_y = 1; // y轴向设置方向
	...
	for (i = 0; i < m_ncAllCodeWord; ++i)
	{
		for (j = 0; j < 8; ++j)
		{
			...
			while (m_byModuleData[x][y] & 0x20); // 除了功能模块
...
void CQR_Encode::SetMaskingPattern(int nPatternNo)
{
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize; ++j)
		{
			if (! (m_byModuleData[j][i] & 0x20)) // 除了功能模块
			{
				BOOL bMask;
...
void CQR_Encode::SetFormatInfoPattern(int nPatternNo)
{
	...
	// 左部下侦查样式权利安排
	m_byModuleData[8][m_nSymbleSize - 8] = '\x30'; // 固定的黑暗模块
...
int CQR_Encode::CountPenalty()
{
	...
	// 1:1在相同线： 3:1 ： 1比率(暗:明:暗:明:暗)样式
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize - 6; ++j)
		{
			if (((j == 0) ||				 (! (m_byModuleData[i][j - 1] & 0x11))) && // 外部明或标志
											 (   m_byModuleData[i][j]     & 0x11)   && // 暗- 1
											 (! (m_byModuleData[i][j + 1] & 0x11))  && // 明- 1
											 (   m_byModuleData[i][j + 2] & 0x11)   && // 暗- 1
											 (   m_byModuleData[i][j + 3] & 0x11)   && // 暗- 1
											 (   m_byModuleData[i][j + 4] & 0x11)   && // 暗- 1
											 (! (m_byModuleData[i][j + 5] & 0x11))  && // 明- 1
											 (   m_byModuleData[i][j + 6] & 0x11)   && // 暗- 1
				((j == m_nSymbleSize - 7) || (! (m_byModuleData[i][j + 7] & 0x11))))   // 外部明或标志
			{
				// 在明样式的4或更多前后
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize - 6; ++j)
		{
			if (((j == 0) ||				 (! (m_byModuleData[j - 1][i] & 0x11))) && // 外部明或标志
											 (   m_byModuleData[j]    [i] & 0x11)   && // 暗- 1
											 (! (m_byModuleData[j + 1][i] & 0x11))  && // 明- 1
											 (   m_byModuleData[j + 2][i] & 0x11)   && // 暗- 1
											 (   m_byModuleData[j + 3][i] & 0x11)   && // 暗- 1
											 (   m_byModuleData[j + 4][i] & 0x11)   && // 暗- 1
											 (! (m_byModuleData[j + 5][i] & 0x11))  && // 明- 1
											 (   m_byModuleData[j + 6][i] & 0x11)   && // 暗- 1
				((j == m_nSymbleSize - 7) || (! (m_byModuleData[j + 7][i] & 0x11))))   // 外部明或标志
			{
				// 在明样式的4或更多前后
